...
Пайплайн для анализа клоногенного теста на 6-well планшетах.

В репозитории есть два независимых режима:

- `scan` — старый строгий Java-пайплайн для TIFF-сканов;
- `photo` — Python/OpenCV-пайплайн для фотографий планшетов (`jpg`, `jpeg`, `png`, `tif`, `tiff`).

## Что делает проект

1. Находит лунки на изображении.
...
## Фото планшетов: рекомендуемый режим для новых пользователей

Для фотографий используйте:

```bash
python scripts/analyze_clonogenic_photo.py "data/photos/*.jpg" \
  --output-dir results/photo_run \
  --min-area 50 \
  --replicate-layout auto
```

Главный интерфейс для людей без программирования — ноутбук:

- `colab/Clonogenic_Photo_Pipeline_Colab.ipynb`

В нём все настройки вынесены в самое начало.

### Как называть файлы фотографий

Лучше использовать латиницу, цифры, `_` или `-`. Десятичную дробь можно писать через запятую или точку.
...
HF_0gy_0.jpg
HF_2gy_0,075.jpg
A549_HF_4gy_0.15.jpg
```

Две концентрации на одном планшете, по 3 повторности:

```text
<sample>_<dose>gy_<right-column concentration>_<left-column concentration>.jpg
```
...
HF_0gy_0,3_0,15.jpg
HF_2gy_0_0,075.jpg
A549_HF_4gy_0.3_0.15.jpg
```

Для вертикальной фотографии 6-well планшета лунки нумеруются так:
...
В режиме с двумя концентрациями:

- `W2`, `W4`, `W6` = концентрация справа, первая концентрация в имени файла;
- `W1`, `W3`, `W5` = концентрация слева, вторая концентрация в имени файла.

Если в имени файла указана только одна концентрация, все 6 лунок автоматически считаются повторностями одного условия.

### Самые важные параметры фотоанализа

Обычно начинать надо с `--min-area 50`.

- `--min-area`: минимальный размер колонии в пикселях. Увеличьте, если считается слишком много мелкого мусора. Уменьшите, если пропадают маленькие настоящие колонии.
- `--min-mean-blackhat`: минимальная локальная темнота/контраст. Увеличьте, если проходят бледные пятна фона. Уменьшите, если пропадают слабо окрашенные колонии.
- `--max-area`: верхняя граница обычной одиночной колонии. Более крупные, но сильно окрашенные объекты считаются как слипшиеся колонии (`large_clump`) и не выбрасываются автоматически.
- `--replicate-layout auto|split-columns|all-wells`: раскладка повторностей. Обычно оставляйте `auto`.

На выходе для каждой фотографии создаются:

- `plate_overlay.png` — исходное фото с границами, номерами лунок, контурами и номерами колоний;
- `well_XX_overlay.png` — отдельная лунка с номерами колоний;
- `well_counts.csv` — счёт по лункам;
- `segments.csv` — каждая учтённая область, её номер, площадь, интенсивность, тип (`single`, `large_clump`, `split_clump`);
- `group_summary.csv` — среднее по концентрации внутри фотографии.

### CDI и heatmap для фото

CDI можно строить, если в наборе есть контроль:

- доза `0 Gy`;
- концентрация `0`;
- все нужные комбинации доз и концентраций.

Команда:

```bash
python scripts/build_cdi_table.py \
  --input "4T1=results/photo_run" \
  --material "HF" \
  --output-prefix results/photo_cdi \
  --skip-verification

python scripts/heatmap.py \
  --input results/photo_cdi_long.csv \
  --output results/photo_cdi_heatmap.png \
  --material "HF" \
  --value-field cdi_100
```

`--input "4T1=..."` задаёт название клеточной линии. Для другой линии поменяйте `4T1` на `A549`, `HCT116` и т.д.

## Скан планшетов: Java/TIFF режим

Этот режим предназначен только для `.tif` / `.tiff` сканов.
...
Для фото используйте `Clonogenic_Photo_Pipeline_Colab.ipynb`. В нём:

- в первой настройочной ячейке меняются `CELL_LINE_NAME`, `MATERIAL_NAME`, `MIN_COLONY_AREA`, `MIN_MEAN_BLACKHAT`;
- фотографии загружаются ZIP-архивом;
- результаты скачиваются ZIP-архивом;
- CDI и heatmap строятся автоматически, если `RUN_CDI = True` и в данных есть контроль.

Для сканов используйте `Clonogenic_Scan_Pipeline_Colab.ipynb`.

Как использовать scan-ноутбук:
...
1. Не ломать scan/TIFF режим при доработке photo-пайплайна.
   Фото анализируется отдельным Python-скриптом, Java-код для сканов должен оставаться совместимым со старыми запусками.
